...
虽然渲染黑洞有多种方法，但这些方法都是以光线追踪和时空扭曲为基础的。光线追踪核心思想比较简单，即计算光线路径在场景中的各个点交互产生的颜色值。在传统的光线追踪算法中，光线是沿直线传播的，因此可以用直线来模拟光线路径，而在黑洞强引力的情况下，光线会因为引力透镜效应而产生弯曲，这种现象可以用爱因斯坦的场方程来描述[^eq]：

{{<math>}}R_{\mu\nu}-\frac{1}{2}g_{\mu\nu}=\frac{8\pi G}{c^4}T_{\mu\nu}{{</math>}} 

尽管爱因斯坦方程的形式看起来很简单，实际上他们是一组复杂的二阶非线性微分方程,并没有通解。但对于一些比较特殊的情况，比如史瓦西解，所对应的几何是一个是静止不旋转、不带电荷之黑洞。比如这篇文章[^3]就使用史瓦西解来模拟光线路径。

本文将使用更简单的方法，用万有引力来让光子产生加速度，结合Ray Marching方法模拟光线路径积分。虽然让光子的速度产生变化违背了物理学中光速的不变原则，但模拟出来的光子路径与史瓦西解的差异并不大。
